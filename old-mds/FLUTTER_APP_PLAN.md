...
- Expansion onboarding (`OnboardingScreen`, `createExpansionFranchise`): the GM (not the coach — question.md decision 24) names themselves and the club, picks a home city and conference, and gets a generated coach plus a weak generated roster. The form shows the chosen conference's existing 10 teams as a checkbox list, one pre-checked at random per conference, labeled "Choose the team to replace" (question.md decision 31) — the GM's choice is recorded on `Franchise.replacedTeamAbbreviation`. Still open: actually removing that team from a running league (so only 19 AI teams remain) needs the `League` runtime concept (Phase 2) that doesn't exist yet — today this is bookkeeping only, the league isn't actually held at 20 teams once a franchise exists.
