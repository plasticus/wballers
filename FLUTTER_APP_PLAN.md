...
- Provide coach editing for individual player and coach appearances, with an accessible fallback portrait. Fallback portrait is built (`PortraitImage`'s placeholder state); the actual editor UI is still open.
...
- Portrait edits persist correctly, and rendering tests cover layering, recoloring, weights, and missing assets. (Rendering/recoloring/layering/weights-loading are all tested against the real bundled assets; there's no editor yet, so "edits persist" isn't testable until one exists.)
