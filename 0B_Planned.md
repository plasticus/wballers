...
- **Possession-based engine.** Core possession loop, fouls/free throws, and the full quarters-and-scoreboard game loop are all **built** — see `0A_Completed.md`'s three match-engine entries (`simulatePossession`, `resolveTipOff`, `simulateMatch`). Alternating possession falls out of the engine automatically. A first round of pacing/scoring/bench-rotation calibration is done too (same entry) — combined score down to ~201 average from ~420, bench players now actually see the floor — though it's still not exact and the full "batch-simulate thousands of games against real WNBA numbers" pass is still open. Overtime is built too (5-minute periods until the tie breaks — a season simulator needs a winner out of every game). Still open here: a live/paced play-by-play presentation of the event log (the engine currently just returns the whole game's events at once, not a feed you watch unfold).
  - **Scoring is still running hot (2026-08-18, a direct GM report from watching real games live).** Both teams average too many points per game — the ~201 combined figure above is itself already well above where it should land. Real WNBA reference, by season:

    | Season | Mean team score (PPG) | Mean combined game total |
    | --- | --- | --- |
    | 2026 | 87.4 | ~174.8 |
    | 2025 | 81.7 | ~163.4 |
    | 2024 | 81.7 | ~163.4 |
    ...
    The ask: an occasional shootout climbing past 100 is fine and good (real games do that too) — the target is the *mean* across many simulated games, which should land under 90 per team (well under the current ~201 combined / ~100+ per team implied by it). Explicitly deferred rather than tuned live off one GM's read of a handful of games — this needs the same real "batch-simulate many games, compare the real distribution against these numbers" pass the exit-criteria bullet above already calls for, not a knob turned based on a couple of observed final scores.
  - **Occasional breakaway/quick transition (2026-08-18, a direct GM ask).** A fast-break possession — off a live turnover or defensive rebound, before the defense sets — reading and playing distinctly from the current engine's uniform halfcourt-possession shape. Not designed yet: needs its own trigger condition (how a possession qualifies as a breakaway), pacing (a real fast break resolves in seconds, not the engine's normal possession-length distribution), and play-by-play/translator support (`live_beat_translator.dart` would need a genuine "transition" beat category, not just faster-clock-drain narration reused from the halfcourt phrase banks). Worth scoping alongside the scoring-balance work above, not in isolation — a breakaway is inherently a *high-efficiency* scoring look, so adding more of them without also addressing overall scoring volume would push in the wrong direction.
