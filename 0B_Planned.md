...
- **Player development/training, decided design (2026-08-06)** — captured as design intent (like the injury rework below), not yet built. Grew out of a proposal doc (`training.md`, now folded in here and deleted) grounded in what already existed: `PlayerRatings.potential` as a ceiling, `CoachStats.development`'s doc comment already earmarking it for this, the Work-ethic/development trait cluster (High/Low Potential, Highly Coachable/Stubborn, Gym Rat), `RosterStatus.developmental`, and `star_system.md`'s "Breakout Dilemma" note (a player's rating can rise mid-season from real performance, not just an offseason menu).

  **Two growth channels.** Performance-driven growth is a function of **gap to potential** (more room = more growth), the age curve below, **minutes played** (reps — a bench player gets little from this regardless of talent), the assigned coach's `development` stat, and traits (High/Low Potential and Highly Coachable/Stubborn scale the coach's effect; Gym Rat adds a smaller floor independent of coach quality, per its own description). The `RosterStatus.developmental` slot applies a flat multiplier on top of this for eligible players (`yearsOfService <= 3`) — reusing the existing roster status rather than inventing a new one.

  **Age curve:** fastest growth 20-23, tapering through 26-27, plateau 27-29, decline starting ~30-32 — softened but not eliminated by Gym Rat. No exemptions for anything else, including traits like Leader kept around for locker-room value on an aging veteran — "everyone declines the same" was the explicit call.

  **Weekly cadence, not end-of-season.** Resolves once a week rather than once a season — going a whole season without seeing progress would be brutal for the GM. Training happens on the season's off day: games are Sunday/Thursday (sometimes Tuesday in the postseason), so **training results land Saturday** — the GM advances through Friday, and a training report shows up in the News feed (`0B_Planned.md`'s "news" item, still just a name on a list today) to open and read. Minutes played that week feeds the formula directly, so injuries/missed games reduce growth automatically by reducing counted minutes — no separate injury-specific penalty needed.

  **A real Training screen, not fully automatic.** Two levels of GM control: a **team-wide broad goal** (Offense / Defense / Physical / Balanced) sets the default direction for the whole roster, and **3 training coaches** — a new staff role, distinct from the single head `Coach` and not yet modeled — each work with individual players and can point in a different direction than the team plan (same O/D/Phys/Bal categories, or a specific skill/rating to hyper-focus on). Settings are sticky: whatever's set stays in effect week to week until the GM changes it, so nothing needs to be touched every week to keep working. An Assistant GM that proactively recommends changes ("Player 5 looks like they've plateaued near their potential, someone else could use that individual training slot") is explicitly future work, not this pass.

  **`potential` can move, rarely.** For most players (~90%) it never changes, or shifts by about ±2 over a whole career at most. What can move it: a trade (either direction), a season of too few minutes (down) or heavy minutes for a promising youngster (up), and earning a nickname or a new hair color through the earned-identity system (`0C_Vision_and_Ideas.md`) — up.

  **Breakout/decline moments route through earned identity, not a training-specific mechanic.** A big breakout stretch or a standout postseason run should trigger the existing nickname/hair-color system, not a separate training notification — that system already exists for "something big just happened to this player." Whenever it fires, it needs a surfaced moment, likely the same News feed the weekly training report lives in. Ordinary week-to-week growth doesn't need its own notification; the GM can just check the roster.

  **Not designed yet:** the exact formula/weights (a rough magnitude gut-check landed on: young + high-potential + starter minutes + a strong coach ≈ +4 to +7 overall across a season; real minutes without the Developmental slot ≈ +2 to +4; bench/low-minutes ≈ +0 to +1; past the decline threshold ≈ -1 to -3, steeper further past it); the training-coach staff model itself (hiring/firing, cost, how it's distinct from or related to the head coach); how a Saturday training day fits into the `GameDay`/season-schedule model (a new enum value, or a separate concept entirely outside `ScheduledGame`); and the News feed/section itself, which doesn't exist as anything beyond a name on a list yet.
- **Coach archetypes: built (2026-08-06)** — see `0A_Completed.md`'s coach-archetype entry. GM confirmed keeping all 5 `CoachStats` (not trimming to 3) with archetype biasing them, same shape as the player system. Still ahead: any real hire/fire flow -- one coach per franchise, generated at onboarding, is still the only way a coach enters the game.
