...
- **Season-progression model: decided (2026-08-05) — Option B.** The GM plays through their own team's games one at a time; the other 19 teams' games simulate automatically in the background. `simulateSeason`/`simulateMatch` (the whole backend built this session) stay an admin/testing tool for now, not the primary game loop — later, once the GM has staff, they'll be able to delegate individual games to a Coach, but that's a future phase, not this one.
  **Franchise/save wiring: built (2026-08-06)** — see `0A_Completed.md`'s season-progress entry. `SeasonProgress` (schedule + played-game history + next week) is now a required part of the save, generated once at franchise creation same as `League`, and `advanceOneWeek` is the team-agnostic "background-sim everyone scheduled that week" building block. Still open: the actual per-game flow that gives the GM's own game special treatment (vs. every other team's, which this already handles), and any UI trigger for it at all -- `advanceOneWeek` is a pure function nothing in the app calls yet.
