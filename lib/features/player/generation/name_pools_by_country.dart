import '../domain/country.dart';

/// Country-specific given/surname name pools, transcribed from a
/// GM-provided CSV (2026-08-10, `~/Downloads/wnba_names_final_v3.csv` --
/// not itself part of this repo) covering all 20 [Country] values: USA's
/// own started at 250 given + 150 surnames, then grew twice more
/// (2026-08-19): the old coach-only pool folded in (271/182 -- see
/// [kAllGivenNames]'s own doc comment), then a second GM-provided batch
/// on top of that (537/564, "Goldthwaite" thrown in on its own explicit
/// ask) -- ~25 given + ~25 surnames for every other country, untouched
/// by either addition. Purely invented --
/// not drawn from any real athlete's name -- combined at random by
/// `player_generator.dart`, so any resemblance to a real person's full
/// name is coincidental. Replaces the old flat `core/generation/
/// name_pools.dart` pools, which originally covered players *and*
/// coaches both, specifically to fix same-surname roster collisions the
/// old single shared 42-name pool kept producing for players
/// (`TODO.md`'s player-name-pool item) -- coach generation kept drawing
/// from the old flat pool for a while after that split, on the reasoning
/// that coaches have no [Country] of their own to key country-specific
/// pools off, until a direct GM catch (2026-08-19) called that split
/// itself out as arbitrary: "I didn't know they had a different pool
/// than players?! That's dumb." [kAllGivenNames]/[kAllSurnames] (below)
/// are what coach generation draws from now instead.
const kGivenNamesByCountry = <Country, List<String>>{
  Country.australia: [
    'Lauren',
    'Ezi',
    'Liz',
    'Penny',
    'Steph',
    'Bec',
    'Alanna',
    'Kristy',
    'Cayla',
    'Marianna',
    'Sami',
    'Tiana',
    'Darcee',
    'Isobel',
    'Jade',
    'Shyla',
    'Alice',
    'Anneli',
    'Maddison',
    'Chloe',
    'Sami',
    'Jasmine',
    'Opals',
    'Keely',
    'Molly',
  ],
  Country.belgium: [
    'Emma',
    'Julie',
    'Kyara',
    'Antonia',
    'Elise',
    'Hind',
    'Becky',
    'Maxuella',
    'Serena',
    'Billie',
    'Ine',
    'Laure',
    'Lies',
    'Nastja',
    'Noortje',
    'Katrien',
    'Jitte',
    'Merel',
    'Emilie',
    'Eline',
    'Hanne',
    'Marie',
    'Jana',
    'Heleen',
    'Sien',
  ],
  Country.brazil: [
    'Kamilla',
    'Janeth',
    'Damiris',
    'Clarissa',
    'Erika',
    'Iziane',
    'Adrian',
    'Carla',
    'Raphaella',
    'Taina',
    'Stephanie',
    'Marciane',
    'Patricio',
    'Geisa',
    'Joice',
    'Micaela',
    'Victoria',
    'Alana',
    'Leticia',
    'Sassá',
    'Silvia',
    'Alessandra',
    'Helen',
    'Cintia',
    'Kelly',
  ],
  Country.canada: [
    'Bridget',
    'Natalie',
    'Kia',
    'Shai',
    'Nayo',
    'Miranda',
    'Laeticia',
    'Kayla',
    'Shay',
    'Sami',
    'Mael',
    'Taya',
    'Sarah',
    'Merissah',
    'Aaliyah',
    'Yvonne',
    'Abigail',
    'Cassandre',
    'Syla',
    'Frederique',
    'Kim',
    'Shona',
    'Miah',
    'Teresa',
    'Katherine',
  ],
  Country.china: [
    'Han',
    'Meng',
    'Siyu',
    'Li',
    'Yuan',
    'Xu',
    'Shao',
    'Wu',
    'Zhang',
    'Wang',
    'Chen',
    'Liu',
    'Yang',
    'Huang',
    'Zheng',
    'Sun',
    'Zhu',
    'Guo',
    'Qian',
    'Pan',
    'Li',
    'Chen',
    'Wang',
    'Zhang',
    'Liu',
  ],
  Country.czechRepublic: [
    'Veronika',
    'Katerina',
    'Alena',
    'Michaela',
    'Petra',
    'Lucie',
    'Tereza',
    'Gabriela',
    'Eva',
    'Hana',
    'Marketa',
    'Zuzana',
    'Lenka',
    'Jana',
    'Kamila',
    'Barbora',
    'Klera',
    'Monika',
    'Radka',
    'Adela',
    'Ilona',
    'Michaela',
    'Romana',
    'Jana',
    'Tereza',
  ],
  Country.finland: [
    'Awk',
    'Elina',
    'Linda-Lotta',
    'Sofia',
    'Anni',
    'Sara',
    'Roosa',
    'Lotta-Maj',
    'Helmi',
    'Mari',
    'Kaisa',
    'Merja',
    'Hanna',
    'Minna',
    'Katja',
    'Tiina',
    'Johanna',
    'Pia',
    'Susanna',
    'Riikka',
    'Vilma',
    'Noora',
    'Emmi',
    'Veera',
    'Tuuli',
  ],
  Country.france: [
    'Marine',
    'Dominique',
    'Sandrine',
    'Valeriane',
    'Iliana',
    'Gabby',
    'Sarah',
    'Alexia',
    'Janelle',
    'Mamignan',
    'Valerie',
    'Helena',
    'Pauline',
    'Romane',
    'Karisma',
    'Marie',
    'Carla',
    'Leila',
    'Kadiatou',
    'Janelle',
    'Sandrine',
    'Endene',
    'Celine',
    'Emmanuelle',
    'Aby',
  ],
  Country.germany: [
    'Leonie',
    'Satou',
    'Marie',
    'Sonja',
    'Luisa',
    'Alexis',
    'Romy',
    'Emily',
    'Laura',
    'Frieda',
    'Lena',
    'Sarah',
    'Lisa',
    'Anna',
    'Julia',
    'Franziska',
    'Katrin',
    'Nadine',
    'Melanie',
    'Sabrina',
    'Ina',
    'Jennifer',
    'Romy',
    'Ama',
    'Leonor',
  ],
  Country.greece: [
    'Artemis',
    'Evanthia',
    'Angeliki',
    'Mariella',
    'Ioanna',
    'Pinelopi',
    'Stella',
    'Elena',
    'Afroditi',
    'Anna',
    'Vasiliki',
    'Christina',
    'Georgia',
    'Eleni',
    'Katerina',
    'Danai',
    'Styliani',
    'Zoi',
    'Panagiota',
    'Sofia',
    'Evi',
    'Styliani',
    'Aggeliki',
    'Katerina',
    'Afroditi',
  ],
  Country.hungary: [
    'Dora',
    'Bernadett',
    'Cyesha',
    'Virag',
    'Debora',
    'Reka',
    'Katalin',
    'Agnes',
    'Nikolett',
    'Barbara',
    'Anita',
    'Zsofia',
    'Eszter',
    'Orsolya',
    'Kitti',
    'Viktoria',
    'Flora',
    'Roxana',
    'Kinga',
    'Lilla',
    'Reka',
    'Krisztina',
    'Zsofia',
    'Eszter',
    'Nikolett',
  ],
  Country.italy: [
    'Cecilia',
    'Martina',
    'Francesca',
    'Jasmine',
    'Costanza',
    'Beatrice',
    'Sara',
    'Olga',
    'Valeria',
    'Silvia',
    'Giorgia',
    'Chiara',
    'Elena',
    'Laura',
    'Federica',
    'Caterina',
    'Alessia',
    'Martina',
    'Nicole',
    'Matilde',
    'Gaia',
    'Nicole',
    'Francesca',
    'Chiara',
    'Martina',
  ],
  Country.mexico: [
    'Gabriela',
    'Sofia',
    'Alejandra',
    'Fernanda',
    'Maria',
    'Valentina',
    'Ximena',
    'Camila',
    'Mariana',
    'Victoria',
    'Renata',
    'Daniela',
    'Regina',
    'Valeria',
    'Lucia',
    'Andrea',
    'Paula',
    'Sara',
    'Jimena',
    'Natalia',
    'Paulina',
    'Ana',
    'Carmen',
    'Rosa',
    'Elena',
  ],
  Country.nigeria: [
    'Evelyn',
    'Amy',
    'Promise',
    'Adaora',
    'Murjanatu',
    'Pallas',
    'Sarah',
    'Elizabeth',
    'Victoria',
    'Blessing',
    'Ruth',
    'Patience',
    'Mercy',
    'Joy',
    'Deborah',
    'Hauwa',
    'Nkechi',
    'Chidinma',
    'Ogechi',
    'Chinwe',
    'Nkem',
    'Upe',
    'Ngozi',
    'Chioma',
    'Ify',
  ],
  Country.senegal: [
    'Astou',
    'Oumou',
    'Mame-Marie',
    'Yacine',
    'Bintou',
    'Fatou',
    'Aminata',
    'Sokhna',
    'Ndeye',
    'Khadija',
    'Rokhaya',
    'Aicha',
    'Mariama',
    'Coumba',
    'Mously',
    'Adama',
    'Awa',
    'Korka',
    'Bineta',
    'Seynabou',
    'Fatou',
    'Astou',
    'Oumou',
    'Mame',
    'Bineta',
  ],
  Country.serbia: [
    'Ana',
    'Sonja',
    'Jelena',
    'Nevena',
    'Saša',
    'Dragana',
    'Tamara',
    'Tina',
    'Maja',
    'Aleksandra',
    'Ivana',
    'Katarina',
    'Milica',
    'Jovana',
    'Sanja',
    'Bojana',
    'Olga',
    'Vladana',
    'Sofija',
    'Nina',
    'Dajana',
    'Nevena',
    'Jelena',
    'Ana',
    'Sara',
  ],
  Country.slovenia: [
    'Nika',
    'Teja',
    'Eva',
    'Zala',
    'Tina',
    'Sara',
    'Annamaria',
    'Blanka',
    'Maruša',
    'Aleksandra',
    'Ajša',
    'Manca',
    'Lea',
    'Hana',
    'Kaja',
    'Katja',
    'Tjaša',
    'Urška',
    'Lara',
    'Pia',
    'Anja',
    'Zala',
    'Teja',
    'Larisa',
    'Sandra',
  ],
  Country.spain: [
    'Astou',
    'Alba',
    'Maite',
    'Laura',
    'Cristina',
    'Silvia',
    'Leticia',
    'Queralt',
    'Paula',
    'Maria',
    'Awa',
    'Andrea',
    'Elena',
    'Mariona',
    'Nerea',
    'Iyana',
    'Belen',
    'Helena',
    'Txell',
    'Angela',
    'Lucila',
    'Carmen',
    'Laura',
    'Marta',
    'Georgina',
  ],
  Country.turkey: [
    'Tilbe',
    'Iagupova',
    'Olcay',
    'Pelin',
    'Bahar',
    'Sevgi',
    'Asena',
    'Elif',
    'Kardelen',
    'Inci',
    'Derin',
    'Melis',
    'Cansu',
    'Ayse',
    'Esra',
    'Gizem',
    'Birsel',
    'Cagla',
    'Tugba',
    'Iilayda',
    'Melis',
    'Ayse',
    'Tugce',
    'Pelin',
  ],
  Country.usa: [
    "A'ja",
    'Breanna',
    'Chelsea',
    'Jewell',
    'Skylar',
    'Nneka',
    'Chiney',
    'Candace',
    'Brittney',
    'Diana',
    'Kahleah',
    'Alyssa',
    'Napheesa',
    'Sabrina',
    'Jonquel',
    'Natasha',
    'Dearica',
    'Allisha',
    'Kayla',
    'Kelsey',
    'Tiffany',
    'Sylvia',
    'Tamika',
    'Lisa',
    'Tina',
    'Seimone',
    'Angel',
    'Caitlin',
    'Kamilla',
    'Aliyah',
    'Lexie',
    'Kaitlyn',
    'Sydney',
    'Destanni',
    'Tyasha',
    'Crystal',
    'Rhyne',
    'Jackie',
    'NaLyssa',
    'Shakira',
    'Jasmine',
    'Diamond',
    'Asia',
    'Brianna',
    'Kiah',
    'Layshia',
    'Tierra',
    'Courtney',
    'Kysre',
    'Chennedy',
    'Lexie',
    'Azura',
    'Michaela',
    'Myisha',
    'Kaila',
    'Aerial',
    'Lexie',
    'Teaira',
    'Isabelle',
    'Satou',
    'Odyssey',
    'Theresa',
    'Monique',
    'Dantas',
    'Glory',
    'DeWanna',
    'Bria',
    'Jordin',
    'Imani',
    'Alexis',
    'Kia',
    'Nikki',
    'Tasia',
    'Camille',
    'Elena',
    'Epiphanny',
    'Cappie',
    'Swin',
    'Tanisha',
    'Kara',
    'Taj',
    'Ruth',
    'Chamique',
    'Alana',
    'Clarissa',
    'Erika',
    'Iziane',
    'Taina',
    'Stephanie',
    'Joice',
    'Micaela',
    'Victoria',
    'Alana',
    'Leticia',
    'Sassá',
    'Zoe',
    'Olivia',
    'Ava',
    'Mia',
    'Charlotte',
    'Amelia',
    'Harper',
    'Evelyn',
    'Abigail',
    'Emily',
    'Elizabeth',
    'Sofia',
    'Avery',
    'Ella',
    'Scarlett',
    'Grace',
    'Chloe',
    'Layla',
    'Riley',
    'Zoey',
    'Nora',
    'Lily',
    'Eleanor',
    'Hannah',
    'Lillian',
    'Addison',
    'Aubrey',
    'Ellie',
    'Stella',
    'Natalie',
    'Zoe',
    'Leah',
    'Hazel',
    'Violet',
    'Aurora',
    'Savannah',
    'Audrey',
    'Brooklyn',
    'Bella',
    'Claire',
    'Skylar',
    'Lucy',
    'Paisley',
    'Everly',
    'Anna',
    'Caroline',
    'Nova',
    'Genesis',
    'Emilia',
    'Kennedy',
    'Samantha',
    'Maya',
    'Willow',
    'Kinsley',
    'Naomi',
    'Emily',
    'Hannah',
    'Madison',
    'Ashley',
    'Sarah',
    'Alexis',
    'Samantha',
    'Jessica',
    'Elizabeth',
    'Taylor',
    'Lauren',
    'Alyssa',
    'Kayla',
    'Abigail',
    'Brianna',
    'Olivia',
    'Emma',
    'Megan',
    'Grace',
    'Victoria',
    'Rachel',
    'Anna',
    'Sydney',
    'Destiny',
    'Morgan',
    'Jennifer',
    'Jasmine',
    'Haley',
    'Julia',
    'Kaitlyn',
    'Nicole',
    'Amanda',
    'Katherine',
    'Natalie',
    'Hailey',
    'Alexandra',
    'Savannah',
    'Chloe',
    'Rebecca',
    'Stephanie',
    'Maria',
    'Sophia',
    'Mackenzie',
    'Allison',
    'Isabella',
    'Amber',
    'Mary',
    'Danielle',
    'Gabriella',
    'Jordan',
    'Laura',
    'Cindy',
    'Kimberly',
    'Michelle',
    'Destinee',
    'Tiana',
    'Asia',
    'Briana',
    'Keisha',
    'Latasha',
    'Shauna',
    'Ebony',
    'Jada',
    'Kiana',
    'Tierra',
    'Diamond',
    'Shanice',
    'Shania',
    'Deja',
    'Kiara',
    'Raven',
    'Simone',
    'Aaliyah',
    'Makayla',
    'Nia',
    'Alisha',
    'LaToya',
    'Tasha',
    'Brandy',
    'Tamara',
    'Crystal',
    'April',
    'Monique',
    'Nikki',
    'Tanya',
    'Tasha',
    'Krystal',
    'Keana',
    'Tanesha',
    'Ayanna',
    'Imani',
    'Jordyn',
    'Kylie',
    'Madeline',
    'Maya',
    'Paige',
    'Sierra',
    'Summer',
    'Trinity',
    'Zoe',
    // Folded in from the old coach-only flat pool (2026-08-19, a direct
    // GM ask: "coach names... they should pull from the same pool" --
    // see `kAllGivenNames`'s own doc comment below for the full story).
    // Only the names not already present above -- 25 of the original 46
    // were already duplicates of names already in this list.
    'Chidinma',
    'Fatima',
    'Noor',
    'Priya',
    'Ingrid',
    'Freya',
    'Camila',
    'Valentina',
    'Amara',
    'Nadia',
    'Yuki',
    'Hana',
    'Mei',
    'Aiko',
    'Anh',
    'Linh',
    'Thandiwe',
    'Zanele',
    'Ngozi',
    'Adaeze',
    'Renee',
    // A GM-provided batch of new names (2026-08-19), added straight
    // in as given -- no editorial filtering.
    'Marisol',
    'Celeste',
    'Vivienne',
    'Paloma',
    'Isolde',
    'Serena',
    'Margot',
    'Orla',
    'Beatrix',
    'Saoirse',
    'Chiara',
    'Elodie',
    'Tamsin',
    'Cleo',
    'Vesper',
    'Astrid',
    'Imogen',
    'Petra',
    'Leonie',
    'Sylvie',
    'Reverie',
    'Maren',
    'Odette',
    'Thea',
    'Zinnia',
    'Corinne',
    'Isadora',
    'Brigitte',
    'Catarina',
    'Dagmar',
    'Elspeth',
    'Greta',
    'Helene',
    'Josefine',
    'Katja',
    'Liselotte',
    'Mirela',
    'Nadège',
    'Orsolya',
    'Quirine',
    'Ragnhild',
    'Siobhan',
    'Tuulikki',
    'Ulrike',
    'Vendela',
    'Wiesława',
    'Xenia',
    'Yelena',
    'Zofia',
    'Anneliese',
    'Bríd',
    'Cécile',
    'Dorota',
    'Geneviève',
    'Chantal',
    'Véronique',
    'Nathalie',
    'Céline',
    'Dominique',
    'Amélie',
    'Marigold',
    'Girly',
    'Ashton',
    'Queen',
    'Wilma',
    'Squid',
    'Daffney',
    'Yessica',
    'Jess',
    'Amy',
    'Angela',
    'Barbara',
    'Betty',
    'Brenda',
    'Carol',
    'Carolyn',
    'Catherine',
    'Christine',
    'Cynthia',
    'Deborah',
    'Debra',
    'Donna',
    'Dorothy',
    'Heather',
    'Helen',
    'Janet',
    'Karen',
    'Kathleen',
    'Linda',
    'Margaret',
    'Melissa',
    'Nancy',
    'Pamela',
    'Patricia',
    'Sandra',
    'Sharon',
    'Shirley',
    'Susan',
    'Marge',
    'Abby',
    'Becky',
    'Daisy',
    'Holly',
    'Judy',
    'Kelly',
    'Macy',
    'Molly',
    'Penny',
    'Ruby',
    'Sally',
    'Wendy',
    'Anna-Marie',
    'Betty-Lou',
    'Billie-Jean',
    'Bobbi-Sue',
    'Jo-Ann',
    'Mary-Beth',
    'Mary-Jane',
    'Mary-Kate',
    'Peggy-Sue',
    'Sarah-Jane',
    'Beulah',
    'Clementine',
    'Dixie',
    'Elvira',
    'Eula',
    'Henrietta',
    'Loretta',
    'Luanne',
    'Myrtle',
    'Opal',
    'Pearl',
    'Verna',
    'Velma',
    'Kat',
    'Mare',
    'Adeline',
    'Alice',
    'Alicia',
    'Anastasia',
    'Andrea',
    'Angelica',
    'Anita',
    'Annabelle',
    'Aria',
    'Ariana',
    'Ariel',
    'Athena',
    'Autumn',
    'Bailey',
    'Bethany',
    'Bianca',
    'Blair',
    'Blake',
    'Bonnie',
    'Brittany',
    'Brooke',
    'Cadence',
    'Callie',
    'Cameron',
    'Cara',
    'Carly',
    'Carmen',
    'Cassandra',
    'Cassidy',
    'Catalina',
    'Cecilia',
    'Celia',
    'Cheyenne',
    'Clara',
    'Colette',
    'Cora',
    'Dahlia',
    'Dakota',
    'Dana',
    'Daniela',
    'Daphne',
    'Darlene',
    'Dawn',
    'Delaney',
    'Delilah',
    'Diane',
    'Eden',
    'Edith',
    'Eileen',
    'Elaine',
    'Eliza',
    'Elsa',
    'Erin',
    'Esme',
    'Estelle',
    'Esther',
    'Eva',
    'Evalyn',
    'Evangeline',
    'Faith',
    'Felicity',
    'Fiona',
    'Flora',
    'Florence',
    'Frances',
    'Francine',
    'Gail',
    'Gemma',
    'Georgia',
    'Gianna',
    'Gillian',
    'Gina',
    'Giselle',
    'Gloria',
    'Gwendolyn',
    'Hallie',
    'Harley',
    'Harmony',
    'Heidi',
    'Hope',
    'Irene',
    'Iris',
    'Isla',
    'Ivy',
    'Jacqueline',
    'Jade',
    'Jamie',
    'Jane',
    'Jean',
    'Jenna',
    'Jillian',
    'Joan',
    'Jocelyn',
    'Josephine',
    'Joy',
    'Joyce',
    'Juliana',
    'Juliet',
    'June',
    'Juniper',
    'Justine',
    'Kacey',
    'Karina',
    'Karla',
    'Katelyn',
    'Keira',
    'Kendall',
    'Kira',
    'Kirsten',
    'Kristen',
    'Kristina',
    'Kyla',
    'Lana',
    'Lara',
    'Lorelei',
    'Maeve',
    'Mallory',
    'Matilda',
    'Meredith',
    'Natalia',
    'Nina',
    'Noelle',
    'Octavia',
    'Penelope',
    'Piper',
    'Ramona',
    'Rosalind',
    'Tessa',
    'Valerie',
    'Bex',
  ],
};

/// See [kGivenNamesByCountry]'s doc comment -- same CSV, `Surname` rows.
const kSurnamesByCountry = <Country, List<String>>{
  Country.australia: [
    'Jackson',
    'Magbegor',
    'Cambage',
    'Taylor',
    'Talbot',
    'Allen',
    'Smith',
    'Wallace',
    'George',
    'Tolo',
    'Whitcomb',
    'Mangakahia',
    'Garbin',
    'Borlase',
    'Melbourne',
    'Heal',
    'Kunek',
    'Maley',
    'Rochci',
    'Sharp',
    'Bishop',
    'Cox',
    'Brown',
    'Froling',
    'Batkovic',
  ],
  Country.belgium: [
    'Meesseman',
    'Vanloo',
    'Linskens',
    'Delaere',
    'Ramette',
    'Ben Abdelkader',
    'Massey',
    'Lisowa',
    'Geldof',
    'Vervaet',
    'Vander',
    'Nauwelaers',
    'Resimont',
    'Claessens',
    'Hollants',
    'Smekens',
    'Meertens',
    'Van',
    'Peeters',
    'Wauters',
    'Joris',
    'Leblon',
    'Devliegher',
    'Kesteloot',
    'Benge',
  ],
  Country.brazil: [
    'Cardoso',
    'Arcain',
    'Dantas',
    'dos Santos',
    'de Souza',
    'Castro',
    'Pinto',
    'Monteiro',
    'Monteiro',
    'Paixao',
    'Soares',
    'Marques',
    'Goncalves',
    'Oliveira',
    'Ribeiro',
    'Cavalcanti',
    'Silva',
    'Costa',
    'Azevedo',
    'Moura',
    'Gustavo',
    'Oliveira',
    'Luz',
    'Santos',
    'Ferreira',
  ],
  Country.canada: [
    'Carleton',
    'Achonwa',
    'Nurse',
    'Colley',
    'Raincock',
    'Ayim',
    'Amihere',
    'Alexander',
    'Gilles',
    'Hill',
    'Bouchard',
    'Hansford',
    'Pellington',
    'Russell',
    'Edwards',
    'Anderson',
    'Brossard',
    'Prosper',
    'Swart',
    'Grymek',
    'Gaucher',
    'Thorburn',
    'Langlois',
    'Plouffe',
    'Murphy',
  ],
  Country.china: [
    'Xu',
    'Gao',
    'Wang',
    'Li',
    'Zhang',
    'Chen',
    'Liu',
    'Yang',
    'Huang',
    'Zhao',
    'Wu',
    'Zhou',
    'Xu',
    'Sun',
    'Ma',
    'Zhu',
    'Hu',
    'Guo',
    'Lin',
    'He',
    'Gong',
    'Miao',
    'Chen',
    'Song',
    'Sui',
  ],
  Country.czechRepublic: [
    'Voračková',
    'Elhotová',
    'Bartoňová',
    'Hanušová',
    'Burgrová',
    'Veselá',
    'Vyoralová',
    'Šujanová',
    'Reisingerová',
    'Štěpánová',
    'Březinová',
    'Vacková',
    'Křížová',
    'Novotná',
    'Dvořáková',
    'Černá',
    'Procházková',
    'Kučerová',
    'Veselovská',
    'Horáková',
    'Burgrova',
    'Viteckova',
    'Horakova',
    'Kulichova',
    'Bartonova',
  ],
  Country.finland: [
    'Kuier',
    'Koskimies',
    'Lehtoranta',
    'Yli',
    'Maki',
    'Salminen',
    'Pasanen',
    'Lahtinen',
    'Tuominen',
    'Jarvinen',
    'Nieminen',
    'Virtanen',
    'Korhonen',
    'Makinen',
    'Maki',
    'Hamalainen',
    'Laine',
    'Heikkinen',
    'Koskinen',
    'Jrvinen',
    'Kesanen',
    'Maki',
    'Korpivaara',
    'Tuukkanen',
    'Laine',
  ],
  Country.france: [
    'Johannès',
    'Malonga',
    'Gruda',
    'Vukosavljevic',
    'Rupert',
    'Williams',
    'Michel',
    'Chartereau',
    'Salagnac',
    'Toure',
    'Illiana',
    'Ciak',
    'Paule',
    'Bernies',
    'Badiane',
    'Djekoundade',
    'Leite',
    'Lacy',
    'Sissoko',
    'Diao',
    'Miyem',
    'Dumerc',
    'Tchatchouang',
    'Gaye',
    'Fauthoux',
  ],
  Country.germany: [
    'Fiebich',
    'Sabally',
    'Guenther',
    'Greinacher',
    'Geiselsoder',
    'Peterson',
    'Bahr',
    'Buescher',
    'Schild',
    'Müller',
    'Schmidt',
    'Schneider',
    'Fischer',
    'Weber',
    'Meyer',
    'Wagner',
    'Becker',
    'Hoffmann',
    'Schäfer',
    'Koch',
    'Guenther',
    'Hebecker',
    'Mawuli',
    'Brunckhorst',
    'Greinacher',
  ],
  Country.greece: [
    'Spetsiotou',
    'Maltsi',
    'Nikolopoulou',
    'Fasoula',
    'Diela',
    'Pavlopoulou',
    'Kaltsidou',
    'Botsi',
    'Kosma',
    'Stamati',
    'Papadopoulou',
    'Vlachou',
    'Christinaki',
    'Karagianni',
    'Stilianou',
    'Gkougkoura',
    'Spyridopoulou',
    'Tsichla',
    'Samolada',
    'Gerostergiou',
    'Maltsi',
    'Kaltsidou',
    'Spanou',
    'Fasoula',
    'Kosma',
  ],
  Country.hungary: [
    'Horvath',
    'Hatar',
    'Goree',
    'Kiss',
    'Dubei',
    'Lelik',
    'Varga',
    'Nagy',
    'Kovacs',
    'Szabo',
    'Toth',
    'Molnar',
    'Fekete',
    'Balogh',
    'Meszaros',
    'Simons',
    'Lakatos',
    'Papp',
    'Bogdan',
    'Galgoczi',
    'Krivacevic',
    'Fegyverneky',
    'Simon',
    'Vajda',
    'Kalisz',
  ],
  Country.italy: [
    'Zandalasini',
    'Bestagno',
    'Pan',
    'Keys',
    'Verona',
    'Carangelo',
    'Cinili',
    'Romeu',
    'De',
    'Pezzani',
    'Rossi',
    'Ferrari',
    'Russo',
    'Bianchi',
    'Romano',
    'Gallo',
    'Costa',
    'Fontana',
    'Conti',
    'Esposito',
    'Zandalasini',
    'Andre',
    'Dotto',
    'Consolini',
    'Sottana',
  ],
  Country.mexico: [
    'Gomez',
    'Perez',
    'Lopez',
    'Rodriguez',
    'Hernandez',
    'Martinez',
    'Gonzalez',
    'Garcia',
    'Sanchez',
    'Ramirez',
    'Cruz',
    'Flores',
    'Morales',
    'Vazquez',
    'Jimenez',
    'Reyes',
    'Diaz',
    'Torres',
    'Gutierrez',
    'Ruiz',
    'Mendoza',
    'Aguilar',
    'Ortiz',
    'Alvarez',
    'Castillo',
  ],
  Country.nigeria: [
    'Akhator',
    'Okonkwo',
    'Amukamara',
    'Kalu',
    'Musa',
    'Kunaiyi',
    'Ogoke',
    'Balogun',
    'Chidom',
    'Edeferioka',
    'Adeayo',
    'Onyiah',
    'Ibekwe',
    'Uchendu',
    'Nnamaka',
    'Okoye',
    'Eze',
    'Nwosu',
    'Okafor',
    'Igwe',
    'Atosu',
    'Udeaja',
    'Okonkwo',
    'Ekeruche',
    'Sadiq',
  ],
  Country.senegal: [
    'Traore',
    'Thiam',
    'Sy',
    'Diop',
    'Dieme',
    'Badiane',
    'Fall',
    'Sene',
    'Ba',
    'Ndiaye',
    'Cisse',
    'Diallo',
    'Gueye',
    'Mbodj',
    'Sow',
    'Kane',
    'Seck',
    'Touré',
    'Wane',
    'Mbaye',
    'Dieng',
    'Traore',
    'Sy',
    'Diop',
    'Fall',
  ],
  Country.serbia: [
    'Dabović',
    'Petrović',
    'Brooks',
    'Jovanović',
    'Crnčević',
    'Stanković',
    'Radočaj',
    'Krajisnik',
    'Miljković',
    'Vulić',
    'Mandić',
    'Živković',
    'Đorđević',
    'Marković',
    'Nikolić',
    'Popović',
    'Ilić',
    'Pavlović',
    'Todorović',
    'Antić',
    'Butulija',
    'Milovanović',
    'Stanković',
    'Dabović',
    'Krnjić',
  ],
  Country.slovenia: [
    'Mühl',
    'Oblak',
    'Lisec',
    'Friskovec',
    'Krajnc',
    'Goršič',
    'Jakovina',
    'Slónjak',
    'Senčar',
    'Kozlevčar',
    'Balažic',
    'Novak',
    'Horvat',
    'Kovač',
    'Zupančič',
    'Kralj',
    'Lah',
    'Vidmar',
    'Turk',
    'Božič',
    'Sofrić',
    'Ocvirk',
    'Oblak',
    'Potočnik',
    'Jakovina',
  ],
  Country.spain: [
    'Ndour',
    'Torrens',
    'Cazorla',
    'Gil',
    'Ouviña',
    'Dominguez',
    'Romero',
    'Casas',
    'Gines',
    'Conde',
    'Dembele',
    'Vilaro',
    'Buenavida',
    'Ortiz',
    'Perez',
    'Martin',
    'Arrojo',
    'Alarcón',
    'Alarcón',
    'Salvatierra',
    'Pascua',
    'Abalde',
    'Gil',
    'Xargay',
    'Cruz',
  ],
  Country.turkey: [
    'Nevriye',
    'Şenyürek',
    'Çakır',
    'Cakir',
    'Bilgiç',
    'Çağlar',
    'Uzun',
    'Yalçın',
    'Bayram',
    'Akgün',
    'Güçlü',
    'Yılmaz',
    'Kaya',
    'Demir',
    'Şahin',
    'Çelik',
    'Aydın',
    'Özdemir',
    'Arslan',
    'Doğan',
    'Kılıç',
    'Guner',
    'Kavurmacıoğlu',
    'Canitez',
    'Alben',
    'Yilmaz',
  ],
  Country.usa: [
    'Wilson',
    'Stewart',
    'Gray',
    'Loyd',
    'Diggins',
    'Ogwumike',
    'Parker',
    'Griner',
    'Taurasi',
    'Copper',
    'Thomas',
    'Collier',
    'Ionescu',
    'Jones',
    'Cloud',
    'Hamby',
    'Gray',
    'McBride',
    'Plum',
    'Hayes',
    'Fowles',
    'Catchings',
    'Leslie',
    'Thompson',
    'Augustus',
    'Reese',
    'Clark',
    'Cardoso',
    'Boston',
    'Hull',
    'Davis',
    'Johnson',
    'Smith',
    'Williams',
    'Brown',
    'Moore',
    'Walker',
    'Young',
    'Jackson',
    'Harris',
    'Anderson',
    'Taylor',
    'Martin',
    'White',
    'Garcia',
    'Miller',
    'Robinson',
    'Clark',
    'Rodriguez',
    'Lewis',
    'Lee',
    'Allen',
    'King',
    'Wright',
    'Scott',
    'Green',
    'Baker',
    'Hill',
    'Nelson',
    'Carter',
    'Mitchell',
    'Roberts',
    'Phillips',
    'Evans',
    'Turner',
    'Torres',
    'Parker',
    'Collins',
    'Edwards',
    'Stewart',
    'Flores',
    'Morris',
    'Nguyen',
    'Murphy',
    'Rivera',
    'Cook',
    'Rogers',
    'Morgan',
    'Peterson',
    'Cooper',
    'Reed',
    'Bailey',
    'Bell',
    'Gomez',
    'Murphy',
    'Bailey',
    'Rivera',
    'Cooper',
    'Richardson',
    'Cox',
    'Howard',
    'Ward',
    'Torres',
    'Peterson',
    'Gray',
    'Ramirez',
    'James',
    'Watson',
    'Brooks',
    'Kelly',
    'Sanders',
    'Price',
    'Bennett',
    'Wood',
    'Barnes',
    'Ross',
    'Henderson',
    'Coleman',
    'Jenkins',
    'Perry',
    'Powell',
    'Long',
    'Patterson',
    'Hughes',
    'Flores',
    'Washington',
    'Butler',
    'Simmons',
    'Foster',
    'Gonzales',
    'Bryant',
    'Alexander',
    'Russell',
    'Griffin',
    'Diaz',
    'Hayes',
    'Myers',
    'Ford',
    'Hamilton',
    'Graham',
    'Sullivan',
    'Wallace',
    'Woods',
    'Cole',
    'West',
    'Jordan',
    'Owens',
    'Reynolds',
    'Fisher',
    'Ellis',
    'Harrison',
    'Gibson',
    'Macdonald',
    'Marshall',
    'Wells',
    'Simpson',
    'Stevens',
    'Tucker',
    'Porter',
    'Hunter',
    // Folded in from the old coach-only flat pool -- see the matching
    // note at the end of [kGivenNamesByCountry]'s own USA list above.
    // Only the names not already present above -- 10 of the original 42
    // were already duplicates of names already in this list.
    'Okafor',
    'Adeyemi',
    'Mensah',
    'Osei',
    'Diallo',
    'Traore',
    'Kone',
    'Nakamura',
    'Watanabe',
    'Kim',
    'Park',
    'Tran',
    'Singh',
    'Patel',
    'Sharma',
    'Kaur',
    'Martinez',
    'Hernandez',
    'Lopez',
    'Silva',
    'Costa',
    'Almeida',
    'Petrov',
    'Ivanova',
    'Kowalski',
    'Nowak',
    'Novak',
    'Andersson',
    'Larsen',
    'Johansson',
    "O'Brien",
    'Clarke',
    // A GM-provided batch of new names (2026-08-19), added straight
    // in as given -- no editorial filtering. Includes 'Goldthwaite',
    // thrown in on its own as a direct, explicit GM ask.
    'Abbott',
    'Abernathy',
    'Ackerman',
    'Adderley',
    'Aguilar',
    'Alcott',
    'Aldridge',
    'Andersen',
    'Angelopoulos',
    'Antoniou',
    'Appleton',
    'Archer',
    'Arrington',
    'Ash',
    'Ashford',
    'Atkins',
    'Bakas',
    'Bancroft',
    'Bannister',
    'Barksdale',
    'Barron',
    'Bartholomew',
    'Bauer',
    'Bautista',
    'Baxter',
    'Beauchamp',
    'Beaumont',
    'Becker',
    'Bernard',
    'Bianchi',
    'Black',
    'Blackwood',
    'Blevins',
    'Bolton',
    'Bouchard',
    'Bowers',
    'Braxton',
    'Brennan',
    'Browne',
    'Bruno',
    'Buchanan',
    'Callahan',
    'Calloway',
    'Cameron',
    'Campbell',
    'Cantrell',
    'Carlson',
    'Carmichael',
    'Carrington',
    'Carver',
    'Cash',
    'Cassidy',
    'Castillo',
    'Cervantes',
    'Chambers',
    'Christodoulou',
    'Cohen',
    'Colombo',
    'Colquhoun',
    'Compton',
    'Conti',
    'Conway',
    'Côté',
    'Crookshank',
    'Dalton',
    'Daniels',
    'Daugherty',
    'Davenport',
    'Davies',
    'Dawson',
    'De Luca',
    'Decker',
    'Delacroix',
    'Delgado',
    'Demetriou',
    'Devereux',
    'Dixon',
    'Dominguez',
    'Donovan',
    'Dorsey',
    'Doyle',
    'Driscoll',
    'Dubois',
    'Eastwood',
    'Eaton',
    'Eddington',
    'Edmonds',
    'Elderidge',
    'Elliott',
    'Ellsworth',
    'Emerson',
    'Epstein',
    'Erickson',
    'Espinoza',
    'Esposito',
    'Estes',
    'Everett',
    'Ewing',
    'Fairbanks',
    'Falconer',
    'Faraday',
    'Farley',
    'Farrell',
    'Faulkner',
    'Feldman',
    'Ferrari',
    'Finch',
    'Finley',
    'Fischer',
    'Fitzgerald',
    'Fitzpatrick',
    'Flanagan',
    'Fletcher',
    'Foley',
    'Forbes',
    'Fortin',
    'Franklin',
    'Fraser',
    'Friedman',
    'Friese',
    'Fritz',
    'Fuentes',
    'Gagné',
    'Gagnon',
    'Gallagher',
    'Gallo',
    'Galloway',
    'García',
    'Garner',
    'Garrison',
    'Gauthier',
    'Gentry',
    'Georgiou',
    'Giannopoulos',
    'Gilmore',
    'Giordano',
    'Goddard',
    'Goldberg',
    'Gonzalez',
    'Goodfellow',
    'Gorkowski',
    'Greco',
    'Greene',
    'Grenville',
    'Guerrero',
    'Gutierrez',
    'Hale',
    'Hammond',
    'Hampton',
    'Harding',
    'Hargrove',
    'Harrington',
    'Hartwell',
    'Hastings',
    'Hawthorne',
    'Hendrix',
    'Hensley',
    'Hernandoz',
    'Herrera',
    'Higgins',
    'Hodges',
    'Hoffmann',
    'Hollingsworth',
    'Holloway',
    'Hooper',
    'Hopkins',
    'Houston',
    'Hubbard',
    'Hurst',
    'Hyde',
    'Ibarra',
    'Iglesias',
    'Ingraham',
    'Ingram',
    'Ioannou',
    'Irvine',
    'Irwin',
    'Isherwood',
    'Ives',
    'Jameson',
    'Jankowski',
    'Jasper',
    'Jaxson',
    'Jefferson',
    'Jennings',
    'Jensen',
    'Jesperson',
    'Jimenez',
    'Jonez',
    'Jonson',
    'Kaiser',
    'Kaminski',
    'Kaplan',
    'Karagiannis',
    'Katsaros',
    'Katz',
    'Kaufman',
    'Kearney',
    'Keegan',
    'Kellerman',
    'Kemp',
    'Kendall',
    'Kensington',
    'Kingsley',
    'Klein',
    'Kline',
    'Knox',
    'Koch',
    'Kostopoulos',
    'Kovač',
    'Kozlowski',
    'Lancaster',
    'Landry',
    'Langford',
    'Larimore',
    'Larson',
    'Lavoie',
    'Lawrence',
    'Lawson',
    'Lefevre',
    'Leibowitz',
    'Levy',
    'Lewandowski',
    'Lloyd',
    'Lockwood',
    'Lombardi',
    'Lopes',
    'López',
    'MacDonald',
    'MacGregor',
    'Macias',
    'MacKenzie',
    'MacLeod',
    'Maddox',
    'Mahoney',
    'Makridis',
    'Makris',
    'Malone',
    'Mancini',
    'Manning',
    'Marchetti',
    'Marino',
    'Marquardt',
    'Martínez',
    'Martyn',
    'Maxwell',
    'McDermott',
    'Medina',
    'Melk',
    'Mendoza',
    'Mercer',
    'Merriweather',
    'Meyer',
    'Michalopoulos',
    'Morales',
    'Moreau',
    'Moretti',
    'Morin',
    'Moure',
    'Mueller',
    'Müller',
    'Munro',
    'Myller',
    'Nash',
    'Navarro',
    'Neumann',
    'Neville',
    'Newton',
    'Nichols',
    'Nightingale',
    'Nikolaou',
    'Northwood',
    'Norton',
    'Novák',
    'Nunez',
    "O'Connor",
    "O'Donnell",
    "O'Neill",
    'Ogilvie',
    'Orozco',
    'Ortega',
    'Osborne',
    'Ostrander',
    'Padilla',
    'Palmer',
    'Panagiotopoulos',
    'Papadopoulos',
    'Pappas',
    'Parsons',
    'Pearson',
    'Pemberton',
    'Po',
    'Popescu',
    'Prescott',
    'Quigley',
    'Quinlan',
    'Quinn',
    'Quintana',
    'Qureshi',
    'Radcliffe',
    'Ramsey',
    'Randall',
    'Redmond',
    'Reeves',
    'Renault',
    'Reyes',
    'Ricci',
    'Richter',
    'Rios',
    'Rizzo',
    'Romano',
    'Romero',
    'Rosenberg',
    'Rossi',
    'Rothenstein',
    'Roy',
    'Russo',
    'Samaras',
    'Sampson',
    'Sánchez',
    'Sandoval',
    'Schaefer',
    'Schmidt',
    'Schneider',
    'Schroeder',
    'Schulz',
    'Schwarz',
    'Shapiro',
    'Sinclair',
    'Smithe',
    'Sommers',
    'Stanton',
    'Stanwick',
    'Stern',
    'Sutton',
    'Szymanski',
    'Tanner',
    'Tate',
    'Tayler',
    'Thatcher',
    'Thistlewood',
    'Thommas',
    'Tremblay',
    'Ulloa',
    'Underwood',
    'Upton',
    'Uribe',
    'Valcourt',
    'Valentine',
    'Vanbrough',
    'Vance',
    'Vargas',
    'Vaughan',
    'Vega',
    'Vlahos',
    'Wagner',
    'Walsh',
    'Walton',
    'Watts',
    'Weber',
    'Westbrook',
    'Whitmore',
    'Wilkshire',
    'Willson',
    'Wisniewski',
    'Wolf',
    'Wozniak',
    'Wylliams',
    'Xander',
    'Yarborough',
    'Yates',
    'Yoder',
    'York',
    'Zamora',
    'Zappa',
    'Zellweger',
    'Zielinski',
    'Ziemann',
    'Zimmerman',
    'Zimmermann',
    'Zuniga',
    'Goldthwaite',
  ],
};

/// Every given name across every country, flattened -- what
/// [generateCoach]/[generateTrainingCoaches] draw from (2026-08-19, a
/// direct GM ask: coaches used to have their own small separate 46-name
/// flat pool, entirely disjoint from the player pools above -- "I didn't
/// know they had a different pool than players?! That's dumb. They
/// should pull from the same pool." Folded into the country pools above
/// (Country.usa specifically, since these read as broadly diverse/
/// American names rather than tied to any of the other 19 countries
/// here) and flattened back out here for coach generation, which has no
/// [Country] of its own to key off -- unlike a player, a coach's name
/// doesn't need to correlate with a hometown or anything else generated.
/// `final`, not `const`: built once, lazily, off the const maps above,
/// not re-flattened on every call.
final kAllGivenNames = [
  for (final names in kGivenNamesByCountry.values) ...names,
];

/// See [kAllGivenNames]'s own doc comment -- same story, surnames.
final kAllSurnames = [for (final names in kSurnamesByCountry.values) ...names];
