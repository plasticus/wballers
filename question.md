...
- ~~**Team replacement selection**~~ -- **built later, see decision 31.**
...
## 31. Hands-on-testing fixes: onboarding example text, team replacement, coach portraits, roster row

**Decision:** From another round of hands-on feedback:

- **Onboarding club-name example**: "Des Moines Dirtbags" (decision 26's own fix) read as too much for a first impression; swapped for "New Orleans Brass."
- **Team replacement, un-deferred**: decision 26 punted this on "needs a real League runtime concept to persist which 19 AI teams remain against" -- but the GM's *choice* doesn't need that runtime concept to exist, only to be recorded. Added `Franchise.replacedTeamAbbreviation` (asserted to be one of `kInitialLeagueTeams`, in the same conference as `Franchise.team`) and a checkbox list in onboarding ("Choose the team to replace"), one team pre-checked at random per conference (re-rolled on conference switch, using real `Random()` entropy since it's just a UI suggestion, not simulation state), GM-overridable. Actually removing that team from a running 19-team league is still Phase 2 work -- this is bookkeeping only, same shape as `Coach`/`Trait`/`Achievement` before their consuming systems existed. No backward-compat parsing added for old saves missing this field, per the standing "delete and recreate during pre-release schema churn" preference.
- **Coach portraits looked identical to players'**: found and read the original portrait-creator tool (`player-edit.html`, `mixer.html`, `index.html`, `team.html` -- present at the repo root but not previously consulted for this). It confirmed two things `weights.json` alone didn't capture: a coach's `shoulders` should *always* have a value (the editor never offers a "None" option for it), and there's a diagnostic in that tool flagging a coach with goggles as stale/wrong data. Fixed `generatePortraitAppearance` to take an optional `PortraitManifest` and always assign a uniform-random shoulder for a coach when one is given (no weight table exists for shoulders anywhere, so weighted selection doesn't apply), and to exclude goggles from the accessory pool specifically for coaches. Threaded `PortraitManifest` through `generateCoach`/`createExpansionFranchise`/onboarding the same optional-parameter way `PortraitWeights` was.
- **Roster row cleanup**: dropped the "3★ & below" star-tier label per row (not useful information at a glance); added a subtle per-position accent color behind the position abbreviation (purely decorative -- the abbreviation text already conveys the position, so this doesn't violate the color-isn't-the-only-signal accessibility rule).

Explicitly not done this pass, per the user's own "later" framing: shifting portrait vertical alignment/proportions by position (taller for PF/C) to fake height variation without new art, and a broader roster-screen reorganization.
...
**Decision:** Built `PortraitEditorScreen`, covering every field `portraits.md`'s "Player-facing portrait editing should expose..." paragraph calls for: skin tone, hair style, hair color, eyes, eyebrows, nose, mouth, and accessories for everyone; shoulders, hats, glasses, and facial hair additionally for the coach (gated on `isCoach`, matching the same coach-only gating `PortraitAppearance`'s constructor already enforces). Special/neon hair colors are deliberately left out of the editor too, not just generation -- they're unlock-only per the doc, and there's no unlock system yet to gate them against; adding the picker now would let a GM grant themselves an achievement reward that doesn't exist.

**Domain layer additions, mirroring the existing `Franchise.copyWithLineup` precedent:** `Player.copyWithAppearance`, `Coach.copyWithAppearance`, `Franchise.copyWithCoach`/`copyWithRoster`, and `CurrentFranchiseNotifier.updatePlayerAppearance`/`updateCoachAppearance` (same await-`future`-not-`state.value` care as `updateLineup`). `PortraitAppearance` itself needed a real `copyWith` -- unlike `Player`/`Coach`, where only one caller (the editor) ever needed to replace one field from outside, the editor needs to independently update any of ~12 `PortraitAppearance` fields, several nullable, so it uses the standard sentinel-object pattern (`_unsetSentinel`) to distinguish "leave unchanged" from "explicitly set to null" (e.g. picking "Bald" really does need to set `hair` to `null`, not leave it alone).

**Live preview:** a separate `_LivePreview` widget calls `renderPortraitPng` directly (bypassing `PortraitCache` entirely) so every keystroke's draft doesn't pollute the save's portrait cache with appearances that might never be saved.

**Entry points:** tapping a roster row's portrait/name in `TeamRosterScreen` opens the player editor; a new `_CoachRow` above the roster (with the coach's portrait, name, and an edit icon) opens the coach editor.

**A real test-environment gotcha, not a product bug:** two `testWidgets` in the same file that both do real asset-decode work via `tester.runAsync` can leave the second one stuck forever waiting on `portraitManifestProvider`/`portraitWeightsProvider` -- confirmed via `--plain-name` that the same test passes cleanly in isolation, so this is cross-test contamination in the test harness (looks like stale binary-messenger mock state), not a bug in the editor. Fix: one such test per file (`portrait_editor_screen_player_test.dart` / `portrait_editor_screen_coach_test.dart`), not one shared file with several. `test/support/portrait_test_helpers.dart` now holds the shared franchise fixture and the `runAsync` wait loop.

## 30. Achievements and nicknames: full data model now, triggers wait for Phase 2

**Decision:** The award triggers `FLUTTER_APP_PLAN.md` names -- league MVP, scoring leader, defensive MVP, most defensive disruptions -- all require a full season of tracked per-player stats to determine a winner, which doesn't exist: there's no season simulation, no game-by-game stats, no standings, nothing. So, same reasoning as `CoachStats` (decision 15) and most of `Trait` (decision 21) — define the shape and mechanism now, wire the actual triggers when Phase 2 exists to fire them:

- `Achievement` enum (`lib/features/player/domain/achievement.dart`) with the four named award types, plus `PlayerAchievementRecord` (achievement + a zero-based `season` counter -- `Franchise` has no real season concept yet either, so this is forward-looking data with nothing wired to it).
- `kNicknamePools`/`suggestNickname`/`grantAchievement` (`lib/features/player/generation/nickname_generator.dart`): a curated nickname pool per achievement, deliberately worded to avoid colliding with existing `Trait`/`Archetype` label text (e.g. not "Pickpocket," already a trait). `grantAchievement` returns the record and a suggested nickname but deliberately doesn't decide whether to auto-apply it -- `FLUTTER_APP_PLAN.md` splits that by team (auto-apply for the 19 AI teams, GM's choice for their own), which is a season-end ceremony policy that belongs to Phase 2's code, not this generator, since this layer has no season/team context to make that call correctly.
- `Player.achievements` (list, default empty) and `Player.nickname` (nullable) fields, plus `copyWithNickname`/`copyWithAchievement`, and `CurrentFranchiseNotifier.updatePlayerNickname` (refactored `updatePlayerAppearance`/`updatePlayerNickname` to share one `_updatePlayer(playerId, transform)` helper for the roster-replacement boilerplate, since they were about to duplicate it a third time).

**What's actually usable today, since nothing can organically earn an achievement yet:** the neon/special hair colors already sitting unused in `weights.json` (decision 28 explicitly kept generation from ever rolling them, since portraits.md ties them to achievement unlocks) are now genuinely gated: the special-hair-color picker only appears once `player.achievements.isNotEmpty`. There's no way to make that true in the running app yet outside tests, but the gate itself is real and tested (`portrait_editor_screen_locked_test.dart` / `_unlocked_test.dart`, using a test helper that manually grants an achievement).

**Correction, direct from the user:** an early version of this decision gave the GM a free-text nickname field in `PortraitEditorScreen`, reasoning that the plan didn't explicitly forbid free editing. That was wrong -- a nickname is earned through play, and a GM assigning one to an arbitrary player on a whim undermines the entire "earned identity" premise. Pulled the field back out; `Player.nickname` and `CurrentFranchiseNotifier.updatePlayerNickname` stay as infrastructure for Phase 2's season-end ceremony (apply the suggestion, or the GM's override of *that specific* suggestion), but nothing in the running app can set a nickname today, and that's correct until a season can actually be played.

Deliberately NOT built: any actual triggering logic (nobody can become "the league's scoring leader" without a season to lead), and nickname editing for the coach (nicknames are earned through on-court accomplishments only players have).
